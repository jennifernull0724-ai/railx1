...
export default function VerifyIdentityPage() {
    ...
    async function loadData() {
      try {
        ...
        if (res.ok) {
          ...
          if (data.identity) {
            setFullName(data.identity.fullName || '');
            setIdentityDoc(data.identity.document || null);
          }
        }
  ...
  }, [session, sessionStatus, fullName]);

  // Redirect if not authenticated
  if (sessionStatus === 'unauthenticated') {
    router.push('/auth/login?callbackUrl=/dashboard/contractor/verify/identity');
    return null;
  }
    ...
    try {
      const presignRes = await fetch('/api/upload', {
        ...
        body: JSON.stringify({
          ...
          fileSize: file.size,
          folder: 'contractors',
          subfolder: 'verification/identity',
          fileType: 'document',
        }),
      });
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      router.push('/dashboard/contractor/verify/insurance');
    } catch (err) {
    ...
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-sm text-slate-500">Identity Verification</span>
        </div>
      ...
      <div className="text-center mb-8">
        ...
        <h1 className="text-2xl font-bold text-navy-900 mb-2">Identity Verification</h1>
          ...
          {identityDoc ? (
            <div className="flex items-center justify-between p-4 bg-green-50 border border-green-200 rounded-lg">
              <div className="flex items-center gap-3">
                ...
                <span className="text-green-800 font-medium">{identityDoc.name}</span>
              </div>
              <button
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          disabled={loading || !fullName.trim() || !identityDoc}
