...
export async function middleware(request: NextRequest) {
  ...
  if (requiresAuth(pathname)) {
    ...
    console.log('[MIDDLEWARE] Token:', token ? { email: token.email, isAdmin: token.isAdmin } : 'NO TOKEN');
    ...
    if (requiresAdmin(pathname)) {
      // TEMPORARY: Allow specific admin email (remove after confirming fix)
      if (token.email === 'jennnull4@gmail.com') {
        console.log('[MIDDLEWARE] Admin bypass for jennnull4@gmail.com');
        return NextResponse.next();
      }
